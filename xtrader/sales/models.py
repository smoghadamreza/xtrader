...
class Subscription(models.Model):
    ...
    def have_subscribe(cls, user, packages="general"):
        ...
        subscription = (
            Subscription.objects.filter(
                user=user,
                package__category__in=packages,
                expiry__gte=timezone.now(),
            )
            .order_by("-package__gas_fee")
            .first()
        )
        return subscription
    ...
    def subscribe(cls, user, pack):
        ...
        if wallet.balance < pack.price + 5:
            return {
                "s": 403,
                "m": "موجودی کیف پول کافی نیست (موجودی شما باید ۵ تتر بیشتر قیمت اشتراک باشد)",
            }
        ...
        paid = Payment.pay(
            wallet=wallet,
            amount=pack.price,
            reason=f"خرید اشتراک {pack.name}",
            action="pay",
        )
        if paid:
...
class Payment(models.Model):
    ...
    @classmethod
    def pay(
        cls, wallet, amount, action="pay", reason="", pay2ref=True, txid=""
    ):
        ...
        referral_wallet = Wallet.objects.filter(
            user=referral_profile.referred_by
        ).first()
        if referral_wallet:
            Payment.pay(
