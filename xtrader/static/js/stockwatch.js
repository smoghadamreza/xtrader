...
function portfo() {
    $.ajax({
        ...
        error: function (e) {
            let portfolio_div = document.getElementById('pportfo');
            portfolio_div.innerHTML = '<h1><a href="/settings/"><div style="font-family: IRANSans">ابتدا اکسچنج خود را متصل کنید</div></a></h1>';
        }
    })
}
...
function orders() {
    $.ajax({
        ...
        error: function (err) {
            let orders_div = document.getElementById('orders_place');
            orders_div.innerHTML = '<h1><a href="/settings/"><div style="font-family: IRANSans">ابتدا اکسچنج خود را متصل کنید</div></a></h1>';
        }
    });
    // getAccountStatus();
    //    portfo();
